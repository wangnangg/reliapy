...
param_pair = [(1, 4), (1, 5), (1, 6), (2, 7), (2, 8)]
for f, N in param_pair:
    #pn = pbft_pn_creator(N, f)
    print("solving f=", f, "N=", N)
    ...
    pn.solve()
    pn.set_option(steady_state_method="divide")
    #pn.add_cum_reward_func("holding time", lambda x : x.token("PD"))
    pn.solve()
    for name, val in pn.get_rewards():
        print(name, val)
    print("Acyclic MTTA:", spnp.compute_acyclic_mtta(pn))
